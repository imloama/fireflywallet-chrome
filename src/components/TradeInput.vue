...
<template>
  <div class="trade-input">
    ...
    <div class="input-content">
      <div class="mytrade-card" style="padding:0 10px;">
        <div class="card-content">
          ...
          <div class="flex-row full-width">
            <div class="flex1 available" v-if="!isBuy">
              {{$t('Available')}}{{BaseAsset.code}}: {{BaseBalance.balance.toFixed(7)}}
            </div>
            <div class="flex1 available" v-else>
       ...
       <!-- 买卖按钮 -->
      <div class="flex-row full-width" v-if="needTrust.length ===0 ">
        <div class="flex1 btn-flex">
          <v-btn :class="'full-width btn-reset ' + ( isBuy ? 'btn-green' : 'btn-red' )" flat  @click="clean">{{$t('Reset')}}</v-btn>
        </div>
      ...
      <!--授信操作，如果当前用户没有当前的资产-->
      <div class="flex-row full-width footer-btns" v-if="needTrust.length > 0 && isBuy ">
        <div class="flex1 btn-flex">
